...
var functionDetection = new RegExp("(" + arithmeticalFunctions.map(function (elem) {
  return elem.detect;
}).join("|") + ")", "i");
var maxIterationBeforeError = 25;
...
function solveFunction_(table, fun, it) {
  ...
  if (numbers.length == 1 && operations.length == 0) return numbers[0];else return "[ERROR]";
}
/**
      ...
      if (item.header.length === item.align.length) {
        ...
        for (j = 0; j < l; j++) {
          ...
          for (k = 0; k < row.length; k++) {
            row[k].tokens = []; // mathematic and logic functions
