...
(function (global, factory) {
  ...
  function tableFormatToNumerical(cell) {
    return {
      column: cell.match(/[a-z]+/mi)[0].toUpperCase().split('').reduce( // check if you need 'm' in regex
      function (previus, current) {
        return previus + current.charCodeAt(0) - 64;
      }, 0),
      row: parseInt(cell.match(/[0-9]+/mi)[0])
    };
  }

  var simpleMathOperations = [[{
    operator: "^",
    mathFun: function mathFun(a, b) {
      return Math.pow(a, b);
    }
  }], [{
    operator: "*",
  ...
  var logicalOperations = [{
    operator: "=",
    logOperation: function logOperation(a, b) {
      return a == b ? 1 : 0;
    }
  }, {
    operator: "!=",
  ...
  var arithmeticalFunctions = [{
    operator: "sum",
    detect: "^sum\\([a-z][a-z]*[0-9]+-[a-z][a-z]*[0-9]+\\)",
    dataExtract: new RegExp(/(?<=^sum\().*?(?=\))/, "i"),
    mathFun: function mathFun(data, table, it) {
  ...
  var arithmeticalFunctionDetection = new RegExp("(" + arithmeticalFunctions.map(function (elem) {
    return elem.detect;
  }).join("|") + ")", "i");
  var logicalFunctions = [{
    operator: "if",
    detect: "^if\\(([a-z]+[0-9]+|[0-9]+)\\s*(<=|>=|<|>|=|!=)\\s*([a-z]+[0-9]+|[0-9]+)\\s*,",
    logFun: function logFun(data, table, it) {
      ...
      for (var i = 0, k = 0, brackets = 0; i < fun.length && k < 3; i++) {
        fullCommand += fun[i];
        if (fun[i] == '(') brackets++;else if (fun[i] == ')') brackets--;
        if (fun[i] == ',' && brackets == 0) k++;else ifStatement[k] += fun[i];
      }

      console.log(ifStatement);
      console.log(ifStatement.filter(function (elem) {
        return elem.length == 0;
      }));

      if (brackets != 0 || ifStatement.filter(function (elem) {
        return elem.length == 0;
      }).length > 0) {
        return "[ERROR]";
      }
  ...
  var logicalFunctionsDetection = new RegExp("(" + logicalFunctions.map(function (elem) {
    return elem.detect;
  }).join("|") + ")", "i");
  ...
  function CellCalculator(table, cell, it) {
    if (it === void 0) {
      it = 0;
    }

    if (it > exports.defaults.tableFunctionRecursionNumber) {
      return "[ERROR]";
    } else if (/==.*==/i.test(cell)) {
      // found function
      ...
      functions.forEach(function (fun) {
        var solvedFunction = solveFunction_(table, fun[0].substring(2, fun[0].length - 2).toString()
        /*.replace(/\s/g, '')*/
        , it);
        ...
        console.log(fun);
        cell = [cell.substring(0, fun.index), solvedFunction.toString(), cell.substring(fun.index + fun[0].length)].join(''); // fun.index ... index start of string where the function is
  ...
  function detectAllFunctions(cell) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(cell.matchAll(/(==)(?:(?=(\\?))\2.)*?\1/gmi)), _step; !(_step = _iterator()).done;) {
      fun = _step.value;
      functions.push(fun);
    }
  ...
  function solveFunction_(table, fun, it, logicFun) {
    if (logicFun === void 0) {
      logicFun = false;
    }
    ...
    while (fun.length > 0) {
      ...
      if (numbers.length > operations.length) {
        ...
        logicalFunctions.filter(function (elem) {
          return new RegExp(elem.detect).test(fun);
  ...
  function extractSubFunction(fun) {
    ...
    while (fun.length > 0) {
      ...
      if (openBrackets != 0) {
        if (closedBrackets == openBrackets) break;else {
          subfunction += fun[0];
        }
      }
        ...
        if (item.header.length === item.align.length) {
          ...
          for (j = 0; j < l; j++) {
            ...
            for (k = 0; k < row.length; k++) {
              row[k].tokens = [];
              if (exports.defaults.tableFunctions == true) row[k].text = CellCalculator(item.rows, row[k].text);
